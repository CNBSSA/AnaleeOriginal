...
def test_login_link_includes_absolute_url_when_configured(canary_app, monkeypatch):
    _enable(monkeypatch)
    ...
    assert link["login_url"].startswith("https://analee.test/workspace/enter?token=")
    assert link["url_path"] in link["login_url"]
