"""RS256 JWT verification for the Accountants-Club SSO consumer path (P3).

Analee is a *consumer* of The Accountants hub (`clubhub`): the hub holds the
RS256 private key and signs short-lived access tokens; Analee validates them
here with the hub's **public** key only. Mirrors the hub's `hub/jwt_util.py`
verbatim so the implementations never drift — and adds nothing to the auth
path beyond `cryptography` (already pinned in requirements). No PyJWT, no
OAuth server.

Security: `alg` is pinned to RS256 (so `alg=none`/HS256-confusion is
rejected), the signature is verified **before** the payload is trusted, then
`exp`/`nbf`/`iss`/`aud` are checked.
"""
